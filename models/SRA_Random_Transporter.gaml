...
global{
	...
	//>> these flags are from the stigmergy approach and are not used here, as this model only uses
...
//##########################################################
experiment SRA_Random_Transporter_No_Charts type:gui{
...
experiment SRA_Random_Transporter type: gui {
...
experiment SRA_Random_Transporter_batch type: batch until: (cycle >= 10000) repeat: 40 autorun: true keep_seed: true{
    ...
    ask simulations {
    	...
    	save [int(self), self.seed, disturbance_cycles ,self.cycle, self.total_delivered, mean_cyc_to_deliver]  
          to: "result/SRA_Random_batch_results.csv" type: "csv" rewrite: false header: true; 
    	}       
	}		
}
